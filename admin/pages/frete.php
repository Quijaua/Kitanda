<?php
    ...
    $create = verificaPermissao($_SESSION['user_id'], 'fretes', 'create', $conn);
    $disabledCreate = !$create ? 'disabled' : '';

    $update = verificaPermissao($_SESSION['user_id'], 'fretes', 'update', $conn);
    $disabledUpdate = !$update ? 'disabled' : '';

    $delete = verificaPermissao($_SESSION['user_id'], 'fretes', 'delete', $conn);
    $disabledDelete = !$delete ? 'disabled' : '';
?>

<?php
    // Dimensão padrão (não está no banco)
    $defaultDimensao = [
        'id' => 0,
        'nome' => 'Padrão',
        'altura' => 4,
    ...
    // Buscar dimensões personalizadas
    $stmt = $conn->prepare("SELECT * FROM tb_frete_dimensoes ORDER BY nome");
    $stmt->execute();
    ...
    <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body text-center py-4">
                ...
                <div class="text-secondary">Tem certeza de que deseja excluir a página <b>"<span id="freteNome"></span>"</b>?</div>
            </div>
    ...
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            <div class="col">
                ...
                <div class="text-secondary mt-1">Aqui estão as fretes do sistema.</div>
            </div>
            <!-- Page title actions -->
            <div class="col-auto ms-auto d-print-none">
                <a href="<?= ($create) ? INCLUDE_PATH_ADMIN."adicionar-frete" : "#"; ?>" class="btn btn-info btn-3 <?= $disabledCreate; ?>" <?= $disabledCreate; ?>>
        ...
        <div class="row row-cards">
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <table id="fretes" class="table card-table table-vcenter text-nowrap datatable">
                        <thead>
                            <tr>
                                <th>Nome</th>
                                <th>Altura (cm)</th>
                        ...
                        <tbody>
                            <?php foreach($dimensoes as $d) : ?>
                            <tr>
                                <td class="more-info" data-label="Name"><?= htmlspecialchars($d['nome']) ?></td>
                                <td><?= $d['altura'] ?></td>
                                <td><?= $d['largura'] ?></td>
                                <td><?= $d['comprimento'] ?></td>
                                <td><?= $d['peso'] ?></td>
                                <td class="text-end">
                                    <?php if ($d['id'] == 0): ?>
                                        <em style="height: 36px; display: inline-flex; align-items: center;">Não editável</em>
                                    <?php else: ?>
                                        <span class="dropdown">
                                            <button class="btn dropdown-toggle align-text-top" data-bs-boundary="viewport" data-bs-toggle="dropdown">Ações</button>
                                            <div class="dropdown-menu dropdown-menu-end">
                                                <?php if ($update): ?>
                                                <a class="dropdown-item" href="<?= INCLUDE_PATH_ADMIN . "editar-frete?id={$d['id']}"; ?>">
                                                ...
                                                <button type="button" class="dropdown-item text-danger btn-delete" data-id="<?php echo $d['id']; ?>" data-name="<?php echo $d['nome']; ?>">
                                                    <!-- Download SVG icon from http://tabler.io/icons/icon/edit -->
...
<script>
    $(document).ready(function () {
        ...
        $(document).on('click', '.btn-delete', function () {
            ...
            $('#freteNome').text(elementNameToDelete); // Define o nome no modal
        ...
        $('#confirmDelete').on('click', function () {
            if (elementIdToDelete) {
                $.ajax({
                    url: `<?= INCLUDE_PATH_ADMIN; ?>back-end/delete-frete.php?id=${elementIdToDelete}`,
                    type: 'GET',
